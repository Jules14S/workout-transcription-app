...
def transcribe_text_to_table(text):
    """Convert extracted text into a structured table format without weights."""
    lines = text.split('\n')
    data = []
    max_sets = 0
    ...
    for line in lines:
        ...
        if '/' in line:
            ...
            # Add row to data with sets and extra info
            row = [exercise] + sets + [extra_info]
            data.append(row)

    # Define the columns dynamically based on max_sets and Extra Info
    columns = ["Exercise"] + [f"Set {i+1}" for i in range(max_sets)] + ["Extra Info"]
...
def create_excel(dataframes):
    ...
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        ...
        for df_tuple in dataframes:
            ...
            columns = ["Exercise"] + [f"Set {i+1}" for i in range(len(df.columns) - 2)] + ["Extra Info"]
